...
import { InboxTableView } from "@/components/inbox/inbox-table-view";
import { toast } from "sonner";
import { WifiOff, LayoutList, MessageSquare } from "lucide-react";
...
function InboxPageInner() {
  ...
  useEffect(() => {
    try {
      ...
      const storedView = localStorage.getItem(INBOX_VIEW_MODE_KEY);
      if (storedView === "chat" || storedView === "table") {
  ...
  const handleToggleViewMode = (mode: "table" | "chat") => {
    setViewMode(mode);
    try {
      localStorage.setItem(INBOX_VIEW_MODE_KEY, mode);
    } catch {
      // Ignore
    }
  };
    ...
    <div className="-m-4 flex h-[calc(100vh-3.5rem)] flex-col overflow-hidden sm:-m-6">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border bg-card px-4 py-2">
        ...
        <div className="flex items-center rounded-lg border border-border bg-muted/50 p-0.5">
          ...
          <button
            type="button"
            onClick={() => handleToggleViewMode("chat")}
            className={cn(
              "flex items-center gap-1.5 rounded-md px-3 py-1 text-xs font-semibold transition-all",
              viewMode === "chat"
                ? "bg-primary text-primary-foreground shadow-sm"
      ...
      {/* Main Content Pane */}
      {viewMode === "table" ? (
        <div className="flex-1 overflow-y-auto min-h-0 h-full bg-background">
          <InboxTableView
