...
(() => {
  ...
  function evaluateChatEligibility(chatId, rawPhone, context) {
    ...
    if (!cleanPhone || cleanPhone.length < 10) {
      return {
        ...
        isAllowedToSync: false,
        reason: "\u{1F7E1} Missing valid phone number. Please enter/confirm the 10-digit mobile number."
      };
    }
    ...
    const matchedCustomer = context.customerPhoneMap[cleanPhone] || context.customerPhoneMap[tenDigit] || context.customerPhoneMap[twelveDigit];
    if (matchedCustomer) {
      return {
  ...
  function createSidebarWidget() {
    ...
    <div class="sli-sidebar-header">
      <div class="sli-sidebar-title">
        <span>\u{1F3ED}</span> SLI WhatsApp Sync
      </div>
  ...
  function renderLoginView(container) {
    ...
    <div class="sli-login-card">
      <div style="font-size:32px;margin-bottom:8px;">\u{1F510}</div>
  ...
  async function handlePassKeyLogin() {
    const inputEl = document.getElementById("sli-input-passkey");
    const errorEl = document.getElementById("sli-login-error");
    const btnEl = document.getElementById("sli-btn-connect");
    ...
    chrome.runtime.sendMessage(
      ...
      (res) => {
        if (btnEl) {
          ...
          btnEl.textContent = "\u{1F680} Connect WhatsApp";
        }
  ...
  function renderDashboardView(container) {
    ...
    <div style="display:flex;gap:4px;border-bottom:1px solid #e2e8f0;padding-bottom:6px;">
      ...
      <button id="sli-tab-blocklist" style="background:none;border:none;font-size:11px;font-weight:600;padding:4px 8px;cursor:pointer;color:${activeTab === "blocklist" ? "#059669;border-bottom:2px solid #059669;" : "#64748b;"}">
        \u{1F6E1}\uFE0F Blocklist (${firewallContext.personalBlacklist.size})
      </button>
    </div>
  ...
  `;
    ...
    const toggleEl = document.getElementById("sli-toggle-autosync");
  ...
  function renderActiveChatCard(container) {
    if (!currentChatName) {
      ...
      <div style="padding:16px 8px;text-align:center;color:#64748b;">
        <div style="font-size:24px;margin-bottom:6px;">\u{1F448}</div>
    ...
    if (decision.eligibility === "BLOCKED_GROUP") {
      statusBadgeHtml = `<span class="sli-badge sli-badge-blocked">\u{1F6AB} GROUP CHAT</span>`;
      ...
      statusBadgeHtml = `<span class="sli-badge sli-badge-blocked">\u{1F464} STAFF / COLLEAGUE</span>`;
      ...
      statusBadgeHtml = `<span class="sli-badge sli-badge-blocked">\u{1F512} PERSONAL CONTACT</span>`;
      actionButtonsHtml = `
    ...
    `;
      ...
      statusBadgeHtml = `<span class="sli-badge sli-badge-quarantine">\u{1F7E1} UNKNOWN / NEW LEAD</span>`;
      actionButtonsHtml = `
    ...
    `;
      ...
      statusBadgeHtml = `<span class="sli-badge sli-badge-eligible">\u{1F7E2} ELIGIBLE CRM CUSTOMER</span>`;
      actionButtonsHtml = `
      ...
      <button id="sli-btn-manual-sync" class="sli-btn-primary">\u{1F4E5} Sync this Chat to CRM</button>
      <button id="sli-btn-block-personal" class="sli-btn-secondary">Mark as Personal (Never Sync)</button>
    `;
    }
    const todayStr = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const lastWeekStr = new Date(Date.now() - 7 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
    container.innerHTML = `
    ...
    <div class="sli-section-card" style="margin-top:8px;">
      <div class="sli-section-title">\u{1F4C5} Date-Range Sync</div>
  ...
  `;
    ...
    document.getElementById("sli-btn-manual-sync")?.addEventListener("click", () => {
      const editPhoneEl = document.getElementById("sli-input-phone-edit");
    ...
    document.getElementById("sli-btn-range-sync")?.addEventListener("click", () => {
      const fromEl = document.getElementById("sli-date-from");
      const toEl = document.getElementById("sli-date-to");
      const fromDate = fromEl?.value ? new Date(fromEl.value).getTime() : 0;
      const toDate = toEl?.value ? new Date(toEl.value).getTime() + 24 * 60 * 60 * 1e3 : Date.now();
      const editPhoneEl = document.getElementById("sli-input-phone-edit");
      const targetPhone = (editPhoneEl?.value || phone).replace(/\D/g, "");
      triggerManualChatSync(targetPhone, decision.customerName || currentChatName, fromDate, toDate);
    });
  }
  function renderBlocklistTab(container) {
    ...
    <div class="sli-section-card" style="margin-top:6px;">
      ...
      <div class="sli-blocklist">
        ${items.length === 0 ? '<p style="color:#94a3b8;font-size:11px;text-align:center;padding:8px 0;margin:0;">No personal numbers blocked</p>' : items.map(
      (num) => `
  ...
  `;
    document.getElementById("sli-btn-add-block")?.addEventListener("click", async () => {
      const inputEl = document.getElementById("sli-input-add-block");
    ...
    container.querySelectorAll("[data-del-num]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        const num = e.currentTarget.getAttribute("data-del-num");
        if (num) {
  ...
  function startChatHeaderObserver() {
    setInterval(() => {
      ...
      if (chatId !== currentChatId || realPhone && realPhone !== currentChatPhone || titleText !== currentChatName) {
  ...
  function scheduleAutoSync(phone, name) {
      ...
      if (messages.length > 0) {
        chrome.runtime.sendMessage({
          ...
          payload: {
            ...
            // Last 25 messages in auto-sync
          }
        });
      }
    }, 2500);
  }
  function extractMessagesFromDOM(fromDate, toDate) {
    const messages = [];
    ...
    for (const node of distinctNodes) {
      ...
      const isOut = dataId && dataId.startsWith("true_") || node.classList.contains("message-out") || Boolean(node.closest(".message-out, [class*='message-out']")) || Boolean(node.querySelector(".message-out, [class*='message-out']")) || Boolean(
        node.querySelector(
      ...
      if (!text) {
        ...
        const lines = raw.split("\n").map((l) => l.trim()).filter(
          (l) => l && !l.match(/^\d{1,2}:\d{2}\s*(am|pm)?$/i) && l !== "Forwarded" && l !== "Today" && !l.includes("Messages and calls are end-to-end encrypted")
        );
        if (lines.length > 0) {
          text = lines.join(" ");
        }
      ...
      if (imgEl && imgEl.src && !imgEl.src.includes("avatar") && !imgEl.src.includes("emoji") && !imgEl.src.startsWith("data:image/svg")) {
        mediaType = "image";
        mediaUrl = imgEl.src;
        if (!text || text.includes("Bank Account")) {
          text = text || "\u{1F4F7} [Photo / Image attachment]";
        }
      }
  ...
  function triggerManualChatSync(customerPhone, customerName, fromDate, toDate) {
    ...
    const syncBtn = document.getElementById("sli-btn-manual-sync") || document.getElementById("sli-btn-range-sync");
    if (syncBtn) {
